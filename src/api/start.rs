...
struct PairClaim {
    tag: String,
    superseded: Option<SessionRecord>,
    reclaim: Option<ContainmentReap>,
    _fence: Option<FileLock>,
}
...
fn resolve_launch(paths: &Paths, req: &StartRequest) -> Result<(PathBuf, ResolvedLaunch)> {
...
fn claim_pair(paths: &Paths, req: &StartRequest, workspace: &Path) -> Result<PairClaim> {
