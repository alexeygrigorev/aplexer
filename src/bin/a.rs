...
struct StartArgs {
    ...
    /// Disable the loopback-SSH worker launch that keeps a worker outside
    /// `user@.service` when `a` itself is running over SSH.
    #[arg(long)]
    no_isolated: bool,
...
fn cmd_prune(paths: &Paths, json_output: bool) -> Result<()> {
    ...
    for record in records {
        ...
        let prunable = !workload_alive && (!worker_alive || missing_socket) && (terminal || missing_socket);
        if !prunable {
            retained_count += 1;
            continue;
        }
