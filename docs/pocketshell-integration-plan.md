...
| `session.deleted` | `status` | `metadata.event="session.deleted"`, `metadata.session_id` | no heru analogue; pure metadata event. Now has a real trigger in the implementation: a clean finish (`a kill`, `exit`, Ctrl-D / shell EOF) removes the session's state (worker-side, at finalization), so the phone sees `deleted` — not a lingering `exited` row |
